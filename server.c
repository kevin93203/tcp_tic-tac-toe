...
int main (int argc, char *argv[])
{
	...
	sm_put(User, "kevin", "123");
	sm_put(User, "lisa", "456");
	sm_put(User, "eva", "789");
...
void * serverthread(void * parm)
{
	...
	while(len = recv(tsd, buf, MAXRCVLEN, 0))
    {
		...
		buf[len] = '\0';		
		char arg1[STRLEN], arg2[STRLEN] , arg3[STRLEN];
		char password[STRLEN];
		int n = sscanf(buf, "%s %s %s", arg1, arg2, arg3);
		
		/* Handle 'join' query */
		if(strcmp(arg1, "join") == 0 && arg2 != NULL)
		{
			pthread_mutex_lock(&mut);
			if(sm_exists(User, arg2) == 1)
			{
				sm_get(User, arg2, password, sizeof(password));
				if(strcmp(arg3,password) == 0)
				{
					...
					{
						/*
						if(strcmp(arg3,mapGet(arg2, m)) != 0){
